...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    if p_value_overall < 0.05:
        ...
        for pair_idx, (i, j) in enumerate(pairs, 1):
            ...
            f.write(f"Comparison {pair_idx}: {group1[group_col]} vs {group2[group_col]}\n")
            f.write("-" * 40 + "\n")
            ...
            f.write(f"{group1[group_col]:20s} {int(group1['Correct']):7d}    {int(group1['Incorrect']):9d}\n")
            f.write(f"{group2[group_col]:20s} {int(group2['Correct']):7d}    {int(group2['Incorrect']):9d}\n")
            f.write("\n")
            ...
            if pair_min < 5:
                ...
                f.write("Test: Fisher's Exact Test\n")
                f.write(f"Reason: At least one cell < 5 (minimum: {pair_min})\n")
                f.write(f"Odds Ratio: {oddsratio:.4f}\n")
                f.write(f"p-value: {p_value_pair:.6f}\n")
            else:
                chi2_pair, p_value_pair, dof_pair, expected_pair = stats.chi2_contingency(pair_contingency, correction=True)
                f.write("Test: Chi-squared Test (with Yates' correction)\n")
                f.write(f"Reason: All cells >= 5 (minimum: {pair_min})\n")
                f.write(f"Chi-squared statistic: {chi2_pair:.4f}\n")
                f.write(f"p-value: {p_value_pair:.6f}\n")
            
            # Bonferroni correction
            bonferroni_alpha = 0.05 / len(pairs)
            f.write(f"Bonferroni-corrected alpha: {bonferroni_alpha:.6f} (0.05/{len(pairs)} comparisons)\n")
            
            if p_value_pair < bonferroni_alpha:
                f.write(f"Result: SIGNIFICANT difference (p < {bonferroni_alpha:.6f})\n")
            else:
                f.write(f"Result: NO significant difference (p >= {bonferroni_alpha:.6f})\n")
